...
Твоя задача — зіставити дані резюме кандидата з вимогами, задачами та умовами вакансії, визначити рівень відповідності й повернути один структурований match analysis відповідно до схеми ParsedMatchAnalysis.

Сформуй один узгоджений результат, який враховує обидві професійні перспективи, а не дві окремі оцінки.

Важливо:
- Використовуй нормалізовані дані як основне джерело для аналізу, а сирі тексти — лише для перевірки контексту, уточнення деталей і виявлення можливих пропусків.
- Спирайся тільки на факти, підтверджені в даних резюме та вакансії.
...
- Якщо вимога вакансії явно не підтверджена в резюме, віднеси її до missing_skills або risk_points залежно від її типу.
...
Порядок аналізу:
1. Спочатку визнач strong_matches, partial_matches, missing_skills і risk_points.
2. Потім сформуй reasoning_summary на основі цих висновків.
3. Лише після цього визнач match_score і recommendation.
4. match_score і recommendation мають бути логічним підсумком попередніх полів, а не незалежною оцінкою.
